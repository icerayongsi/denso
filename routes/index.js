...
const username = '1234';
const password = '1234';
// const password = '424625';

router.get('/login', async (req, res) => {
  if (session.userid) {
    res.redirect('/layout');
  } else
    res.render('login', { title: 'Denso', header: 'Login' });
});

var session;

router.post('/loginprocess', encodeUrl, async (req, res, next) => {
  ...
  if (req.body.username == username && req.body.password == password) {
    ...
    res.render('login', { title: 'Denso', header: 'Login Invalid' });
  }
...
router.post('/Dashboard', encodeUrl, async (req, res, next) => {
  ...
  res.render('main/dashboard_index', {
    title: 'Dashboard',
    header: header,
